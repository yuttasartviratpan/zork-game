...
//    INVENTORY, INSPECT, NOTFOUND, EMPTY, INCOMPLETE
    ...
    INFO(CommandInfo.class, "info", ": Get the information of the room, monsters and items (if any in the room)",
            0, true),
    TAKE(CommandTake.class,"take", "<item name> : Take an item specified that is in the room, use \"take all\" to take all items in the room",
            1, true),
    USE(CommandUse.class,"use", "<item name> : Use the specify item",
            1, true),
    ...
    ATTACK(CommandAttack.class,"attack", ": Attack the monster currently in the room",
            0, true),
    GO(CommandGo.class,"go", "<direction (NEWS)> : Move to the next location by giving specific direction",
            1, true),
    ...
    AUTOPILOT(CommandAutoPilot.class,"autopilot", "<file name> : Play this game using the file containing commands for this game",
            1, false),
    HELP(CommandHelp.class,"help", ": Get all the commands available for this game",
            0, false),
    QUIT(CommandQuit.class,"quit", ": Exit from the current map you're playing in. Warning: The progress will not be save " +
            ...
            0, true),
    PLAY(CommandPlay.class,"play", "<map name> : Start the game in a map of specified choice",
            1, false),
    ...
    SAVE(CommandSave.class,"save", "<save name> Record the current progress as the specified name",
            1, true),
    EXIT(CommandExit.class,"exit", ": Terminate the program instance",
            0, false),
    EQUIP(CommandEquip.class,"equip", "<weapon name> : Armed yourself with specified weapon",
            1, true),
